...
const DIRECTIVE_PATTERN =
	/^(?:\/?\s*)?(?:@ts-|ts-|eslint|oxlint|istanbul|c8|v8|prettier|biome|deno-lint|\/?\s*<reference)\b/iu;
 ...
 */
export function formatText(options: FormatTextOptions): FormatTextResult {
	...
	for (let index = 0; index < lines.length; index++) {
		...
		const hasOverWidthLine = original.some(
			(line) => measureLineWidth(line, options.tabWidth) > options.width,
		);
...
function parseLineComment(line: string): LineComment | undefined {
...
function parseBlockComment(
	...
	for (let index = startIndex + 1; index < lines.length; index++) {
		...
		const body = line.match(
			new RegExp(`^${escapedIndent}\\s*\\*\\s?(.*)$`, 'u'),
		);
...
function isSafeProse(bodies: string[]): boolean {
	if (bodies.length === 0) {
		return false;
	}
